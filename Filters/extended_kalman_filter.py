...
# Defines an extended kalman filter, which allows for dynamic filtering of nonlinear models
...
class ExtendedKalmanFilter(BayesianFilter):
    ...
    def filter(self, observations):
        ...
        # At time t_0
        R, Hx, Hr = self.sde.get_extended_kalman_filter_update_params(self.m0, np.zeros(self.meas_dim))

        v = observations[:,0] -  self.sde.measurement(self.m0)
        S = np.matmul(np.matmul(Hx, self.P0), np.transpose(Hx)) + np.matmul(np.matmul(Hr, R), np.transpose(Hr))
        K = np.matmul(np.matmul(self.P0, np.transpose(Hx)), np.linalg.inv(S))
        ...
        for k in range(0, self.n_times-1):
            ...
            f, Q, Fx, Fq = self.sde.get_extended_kalman_filter_pred_params(self.m[:,k], np.zeros(self.state_dim), deltat)
            mm = f
            Pm = np.matmul(np.matmul(Fx, self.P[:,:,k]), np.transpose(Fx)) + np.matmul(np.matmul(Fq, Q), np.transpose(Fq))

            # Update step
            if (k + 1) in self.measurement_indices:
                R, Hx, Hr = self.sde.get_extended_kalman_filter_update_params(mm, np.zeros(self.meas_dim))
                idx = np.where(self.measurement_indices == k+1)[0][0]
                v = observations[:,idx] -  self.sde.measurement(mm)

                S = np.matmul(np.matmul(Hx, Pm), np.transpose(Hx)) + np.matmul(np.matmul(Hr, R), np.transpose(Hr))
                K = np.matmul(np.matmul(Pm, np.transpose(Hx)), np.linalg.inv(S))
    ...
    def get_filter_cov(self, timepoint, observations):
        """
        Returns the covariance of the distribution at chosen timepoint
        Note: used in ebds importance sampling 
        """
        tolerance = 0.00001
        idx = (np.abs(self.times - timepoint)).argmin()
        if np.abs(self.times[idx] - timepoint) > tolerance:
