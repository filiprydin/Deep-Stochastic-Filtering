...
from Filters.hybrid_network import HybridModel
...
class EBDSLSTMFilter(BayesianFilter):
    def __init__(self, sde, times, measurement_indices, parameters, verbose = True):
        ...
        self.re_norm_method = parameters["Renormalisation method"] # Choose 0-4
    ...
    def _format_input(self, input_timestep, n_observations, validation = False):
        ...
        y_input = torch.tensor(Y[:, :n_observations, :], dtype = dtype, device = device).permute(2, 1, 0)
    ...
    def _calculate_and_format_target(self, n, timestep, input_timestep, n_observations, deltat, validation = False):
        ...
        input_seq, input_non_seq = self._format_input(input_timestep + 1, n_observations, validation)
        
        a1, a2, a3, a4, H = self.sde.get_ebds_filter_params(input_non_seq)

        # Latest available observation
        Yk = input_seq[:,-1,:].clone()

        if n == 0 and n_observations == 1:
            # Use only prior and update
            likelihood = self._observation_likelihood_oto(input_non_seq, Yk).view(-1, 1)
            prior_grad, prior_likelihood = self.sde.prior_gradient_torch(input_non_seq)
            
            expanded_measurement = (Yk - self.sde.measurement_batch_torch(input_non_seq)).unsqueeze(2)   
            ...
            # Calculating network gradient consumes a lot of memory -> process in batches
            batch_size = 100000
            num_samples = input_non_seq.size(0)

            network_grad = torch.zeros((num_samples, self.state_dim), dtype = dtype, device = device)
            network_output = torch.zeros((num_samples, 1), dtype = dtype, device = device)

            for start in range(0, num_samples, batch_size):
                end = min(start + batch_size, num_samples)

                input_non_seq_batch = input_non_seq[start:end, :].requires_grad_(True)
                input_seq_batch = input_seq[start:end, :, :].detach().requires_grad_(False)
                network_output_grad = previous_network(input_seq_batch, input_non_seq_batch)
                network_output[start:end, :] = network_output_grad.detach().view(-1, 1)
                network_grad[start:end, :] = torch.autograd.grad(outputs=network_output_grad, inputs=input_non_seq_batch, 
                                                grad_outputs=torch.ones_like(network_output_grad))[0].detach()
            ...
            total_grad = torch.zeros((num_samples, self.state_dim), dtype = dtype, device = device)
            network_output = torch.zeros((num_samples, 1), dtype = dtype, device = device)
            ...
            for start in range(0, num_samples, batch_size):
                ...
                network_output_grad = previous_network(input_seq_batch, input_non_seq_batch)
                network_output[start:end, :] = network_output_grad.detach().view(-1, 1)
                total_grad[start:end, :] = torch.autograd.grad(outputs=network_output_grad, inputs=input_non_seq_batch, 
                                                grad_outputs=torch.ones_like(network_output_grad))[0].detach()
        ...
        start_time = time.time()
        targets = self._renormalise_targets(targets, n, timestep, input_seq, Yk)
    ...
    def _get_filter_integral_importance(self, n, timestep, Y_input, Yk):
        ...
        for i in range(Y_input.size(dim=0)):
            ...
            repeated_obs = observations.repeat(self.mc_n_points, 1, 1)
            ...
            if self.meas_dim == self.state_dim:
                ...
                np_observations = observations.detach().cpu().numpy().T
                np_yk = yk.detach().cpu().numpy()
                n_obs = np.sum(self.measurement_indices < timestep)
                pad_zeros = len(self.measurement_indices) - n_obs
                observations_formatted = np.hstack((np_observations, np.zeros((self.meas_dim, pad_zeros))))
            ...
            if timestep == 0:
                ...
                model_log_predictions = torch.log(model(repeated_obs, x).detach().flatten())
    ...
    def _get_filter_integral_normal(self, n, timestep, Y_input, Yk):
        ...
        Returns the filter integrals in the given timepoint with the observations. 
        These are needed in re-normalisation schemes 3 and 4
        Monte-Carlo version. Only works when n=0
        Optimized for internal use via _renormalize_targets, for external use see functions below
        Returns:
        integrals, vector with length = number of observation chains given
        """
        ...
        for i in range(Y_input.size(dim=0)):
            ...
            if timestep == 0:
                ...
                # If at a time with an observation, update model output with observation likelihood
                model = self.models[timestep - 1]
                model_predictions = model(repeated_obs, x).detach().flatten()
    ...
    def _get_filter_integral(self, n, timestep, Y_input, Yk):
        ...
        for i in range(Y_input.size(dim=0)):
            ...
            if timestep == 0:
                ...
                # If at a time with an observation, update model output with observation likelihood
                model = self.models[timestep - 1]
                repeated_obs = observations.repeat(x.size(0), 1, 1)
                model_predictions = model(repeated_obs, x).detach().flatten()
                observation_likelihoods = self._observation_likelihood_mto(x, yk)
                ...
                model = self.models[timestep - 1]
                repeated_obs = observations.repeat(x.size(0), 1, 1)
                predictions = model(repeated_obs, x).detach().flatten()
    ...
    def define_and_train_models(self, data_folder_name, n_train_samples):
        ...
        for j in range(len(self.measurement_indices) - 1):
            for i in range(self.N):
                Network = HybridModel(self.meas_dim, self.state_dim, self.parameters, self.verbose).to(device)
        ...
        for k in range(len(self.measurement_indices) - 1):
            for n in range(self.N):
                ...
                input_train_seq, input_train_non_seq = self._format_input(input_timestep, k + 1, validation = False)
                input_val_seq, input_val_non_seq = self._format_input(input_timestep, k + 1, validation = True)
                ...
                if self.remove_extremes:
                    ...
                    input_train_seq = input_train_seq[mask.any(dim=1),:,:]
                    input_train_non_seq = input_train_non_seq[mask.any(dim=1),:]
                    ...
                    input_val_seq = input_val_seq[mask.any(dim=1),:,:]
                    input_val_non_seq = input_val_non_seq[mask.any(dim=1),:]
                ...
                if torch.any(torch.isinf(target_train)):
                    ...
                    input_train_seq = input_train_seq[mask1,:,:]
                    input_train_non_seq = input_train_non_seq[mask1,:]
                else:
                    pass
                if torch.any(torch.isinf(target_val)):
                    ...
                    input_val_seq = input_val_seq[mask2,:,:]
                    input_val_non_seq = input_val_non_seq[mask2,:]
                else:
                    pass

                start_time = time.time()
                network.train_model(input_train_seq, input_train_non_seq, target_train, input_val_seq, input_val_non_seq, target_val)
                network.eval()
    ...
    def load_saved_models(self, folder_name, only_filter_times = False):
        ...
        for i in range(self.n_times-1):
            input_observations.append((i + 1) // self.N + 1)
        ...
        if not (time_consistent and dimensions_consistent and observations_consistent):
        ...
        for j in range(self.n_times - 1):
            ...
            model = HybridModel(self.meas_dim, self.state_dim, self.parameters, self.verbose, norm_factor).to(device)
    ...
    def get_filter_means_and_integrals(self, timepoints, observations):
        ...
        for i in range(len(timepoints)):
            ...
            observations_formatted = torch_observations[:(n_observations-1), :].repeat(self.mc_n_points, 1, 1)
            ...
            if self.norm_method == "Normal":
                ...
                if timestep == 0:
                    ...
                    # If at a time with an observation, update model output with observation likelihood
                    model = self.models[timestep - 1]
                    model_predictions = model(observations_formatted, x).detach().flatten()
                    predictions = model_predictions
                ...
                if self.meas_dim == self.state_dim:
                    ...
                    m = ekf.get_filter_mean(timepoints[i], observations_formatted)
                    P = ekf.get_filter_cov(timepoints[i], observations_formatted)
                    mean = torch.tensor(m, dtype = dtype, device = device)
                    cov = 2 * torch.tensor(P, dtype = dtype, device = device)
                    x = torch.distributions.MultivariateNormal(mean, cov).sample((self.mc_n_points,))
                ...
                if timestep == 0:
                    ...
                    # If at a time with an observation, update model output with observation likelihood
                    model = self.models[timestep - 1]
                    model_log_predictions = torch.log(model(observations_formatted, x).detach().flatten())
                    predictions = torch.exp(model_log_predictions + update_factor - importance_correction)
    ...
    def _evaluate_filter_nn(self, x, timepoint, observations):
        ...
        input_observations = idx // self.N + 1
        ...
        if idx == 0:
            ...
            # If at a time with an observation, update model output with observation likelihood
            observations_formatted = observations[:(input_observations-1), :].repeat(x.size(dim=0), 1, 1)
            ...
            # If at a time between observations, simply evaluate the model
            observations_formatted = observations[:input_observations, :].repeat(x.size(dim=0), 1, 1)
            model = self.models[idx - 1]
            predictions = model(observations_formatted, x).detach().flatten()
    ...
    def save_models(self, folder_name, only_filter_times = False):
        ...
        specifications_file_path = os.path.join(folder_name, "model_specifications.csv")
        specifications = pd.DataFrame(columns = ["Model id", "Timepoint", "State dimension", "Observation dimension", "Number of observations", "Normalisation factor"])
        ...
        for i in range(len(self.models)):
            ...
            specifications_row = {
                ...
                "Number of observations": (i + 1) // self.N + 1,
