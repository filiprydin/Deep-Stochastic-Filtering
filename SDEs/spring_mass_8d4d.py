...
# Defines a spring-mass system in 8 dimensions with linear measurements on 2 positions and 2 velocities
# dXt = A Xt dt + sigma dWt
# h(x) = x (for selected components)
# A = [0, I; A21, A22]

class SpringMass8d4d(DiffusionSDE):
    def __init__(self):
        self.identifier = "SM8D4D"

        self.measurement_noise_std = 1
        self.state_dim = 8
        self.meas_dim = 4

        self.M = self.state_dim // 2

        self.sigma = 1

        self.A = np.zeros((self.state_dim, self.state_dim))
        self.A[:self.M ,self.M:] = np.identity(self.M)

        # Physical constants
        self.m = np.ones(self.M)
        self.k = 1 * np.ones(self.M + 1)
        self.c = 0.2 * np.ones(self.M + 1)

        # A21
        a21diag = -(self.k[:-1] + self.k[1:]) / self.m
        a21diagu = self.k[1:-1] / self.m[:-1]
        a21diagl = self.k[1:-1] / self.m[1:]
        A21 = np.zeros((self.M, self.M))
        np.fill_diagonal(A21, a21diag)
        ...
        np.fill_diagonal(A22, a22diag)
        self.A[self.M:, self.M:] = A22

        self.A_torch = torch.tensor(self.A, dtype = torch.float32, device=device)
    ...
    def drift(self, x):
        mu = np.matmul(self.A, x)
        return mu
    
    def drift_batch(self, x):
        mu = np.matmul(x, self.A.T)
        return mu
    
    def drift_batch_torch(self, x):
        mu = torch.matmul(x, self.A_torch.t())
        return mu

    def diffusion(self, x):
        sigma = self.sigma * np.identity(self.state_dim)
        return sigma
    ...
    def measurement(self, x):
        h = np.matmul(self.H, x)
        return h
    
    def measurement_batch(self, x):
        h = np.matmul(x, self.H.T)
        return h
    
    def measurement_batch_torch(self, x):
        h = torch.matmul(x, self.H_torch.t())
        return h
    ...
    def get_kalman_filter_params(self, deltat):
        ...
        r_k ~ R_k 
        """
        A = np.identity(self.state_dim) + self.A*deltat
