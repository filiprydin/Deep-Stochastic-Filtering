import numpy as np

from scipy.stats import multivariate_normal

from SDEs.diffusion_sde import DiffusionSDE

import torch
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

# Defines a Stochastic Lotka-volterra model in three dimensions

class LotkaVolterra3(DiffusionSDE):
    def __init__(self):
        self.identifier = "LV3"

        self.measurement_noise_std = 0.5

        self.alpha1 = 1
        self.beta1 = 1
        self.gamma = 2

        self.alpha2 = 0.75 
        self.beta2 = 1 

        self.R0 = 1
        self.R0_std = 0.2 # 0.1
        self.M0 = 1.5
        self.M0_std = 0.3 # 0.15
        self.F0 = 0.5
        self.F0_std = 0.1 # 0.05

        self.state_dim = 3
        self.meas_dim = 3

        self.sigma1 = 0.8 # 0.5
        self.sigma2 = 1 # 0.5
        self.sigma3 = 0.5 # 0.5

        self.invertible_meas = False

    def drift(self, x):
        mu = np.zeros(3)
        mu[0] = x[0] * (self.alpha1 - self.alpha2 * x[2])
        mu[1] = x[1] * (self.beta1 - self.beta2 * x[2])
        mu[2] = x[2] * (self.alpha2 * x[0] + self.beta2 * x[1] - self.gamma)
        return mu
    
    def drift_batch(self, x):
        mu = np.zeros((x.shape[0], self.state_dim))
        mu[:,0] = x[:,0] * (self.alpha1 - self.alpha2 * x[:,2])
        mu[:,1] = x[:,1] * (self.beta1 - self.beta2 * x[:,2])
        mu[:,2] = x[:,2] * (self.alpha2 * x[:,0] + self.beta2 * x[:,1] - self.gamma)
        return mu
    
    def drift_batch_torch(self, x):
        mu = torch.zeros((x.shape[0], self.state_dim), dtype = x.dtype, device=device)
        mu[:,0] = x[:,0] * (self.alpha1 - self.alpha2 * x[:,2])
        mu[:,1] = x[:,1] * (self.beta1 - self.beta2 * x[:,2])
        mu[:,2] = x[:,2] * (self.alpha2 * x[:,0] + self.beta2 * x[:,1] - self.gamma)
        return mu

    def diffusion(self, x):
        sigma = np.zeros((self.state_dim, self.state_dim))
        sigma[0,0] = self.sigma1 * x[0]
        sigma[1,1] = self.sigma2 * x[1]
        sigma[2,2] = self.sigma3 * x[2]
        return sigma
    
    def diffusion_batch(self, x):
        sigma = np.zeros((x.shape[0], self.state_dim, self.state_dim))
        sigma[:,0,0] = self.sigma1 * x[:,0]
        sigma[:,1,1] = self.sigma2 * x[:,1]
        sigma[:,2,2] = self.sigma3 * x[:,2]
        return sigma

    def measurement(self, x):
        h = x
        return h
    
    def measurement_batch(self, x):
        h = x
        return h
    
    def measurement_batch_torch(self, x):
        h = x
        return h
    
    def measurement_inverse(self, y):
        raise ValueError("SDE not invertible")
        return y

    def measurement_inverse_batch(self, y):
        raise ValueError("SDE not invertible")
        return y
    
    def get_kalman_filter_params(self, deltat):
        """
        Returns parameters needed for the kalman filter
        Based on model: 
        x_k = A_(k-1) x_(k-1) + q_(k-1) + b_(k-1)
        y_k = H_k x_k + r_k
        q_(k-1) ~ N(0, Q_(k-1))
        r_k ~ R_k 
        """
        raise ValueError("The specified SDE is not linear. Cannot use Kalman Filter")
            
        return A, b, Q, R, H
    
    def get_extended_kalman_filter_update_params(self, x, r):
        """
        Returns parameters needed for the extended kalman filter update step
        Based on model: 
        y_k = h(x_k, r)
        r ~ N(0, R)
        Hx jacobian of h wrt x
        Hr jacobian of h wrt r
        """
        R = np.identity(self.meas_dim) * self.measurement_noise_std**2
        Hx = np.zeros((self.meas_dim, self.state_dim))
        Hx[0,0] = 1
        Hx[1,1] = 1
        Hx[2,2] = 1
        Hr = np.identity(self.meas_dim)
            
        return R, Hx, Hr
    
    def get_extended_kalman_filter_pred_params(self, x, q, deltat):
        """
        Returns parameters needed for the extended kalman filter prediction step
        Based on model: 
        x_k = f(x_k-1, q_k-1)
        q_(k-1) ~ N(0, Q)
        Fx jacobian of f wrt x
        Fq jacobian of f wrt q
        """
        f = np.matmul(np.identity(self.state_dim), x)
        f += self.drift(x)*deltat
        f += np.matmul(self.diffusion(x), q)
        Fx = np.identity(self.state_dim)
        Fx[0,0] += (self.alpha1 - self.alpha2 * x[2]) * deltat
        Fx[0,2] += - self.alpha2 * x[0] * deltat
        Fx[1,1] += (self.beta1 - self.beta2 * x[2]) * deltat
        Fx[1,2] += - self.beta2 * x[1] * deltat
        Fx[2,2] += (self.alpha2 * x[0] + self.beta2 * x[1] - self.gamma) * deltat
        Fx[2,0] += self.alpha2 * x[2] * deltat
        Fx[2,1] += self.beta2 * x[2] * deltat
        Fq = self.diffusion(x)
        Q = np.identity(self.state_dim) * deltat
            
        return f, Q, Fx, Fq
    
    def get_unscented_kalman_filter_pred_params(self, deltat):
        """
        Returns parameters needed for the unscented kalman filter prediction step
        Based on model: 
        x_k = f(x_k-1, q_k-1)
        q_(k-1) ~ N(0, Q)
        """
        Q = np.identity(self.state_dim) * deltat
            
        return Q
    
    def get_unscented_kalman_filter_update_params(self):
        """
        Returns parameters needed for the unscented kalman filter update step
        Based on model: 
        y_k = h(x_k, r)
        r ~ N(0, R)
        """
        R = np.identity(self.meas_dim) * self.measurement_noise_std**2
            
        return R

    def get_ebds_filter_params(self, x):
        """
        Returns parameters needed for the energy-based deep splitting filter
        Input: 
        x: (n_points, state_dim) a collection of states
        Let a = sigma*sigma^T
        returns 
        a1: da_ij/dx_i matrix
        a2: d^2 a_ij/dx_i*dx_j matrix
        a3: dmu_i/dx_i vector
        a4: mu vector
        H: Jacobian of the measurement function 
        """
        n_points = x.size(dim=0)

        a1 = torch.zeros((n_points, self.state_dim, self.state_dim), dtype=x.dtype, device=device)
        a1[:,0,0] = 2 * self.sigma1 ** 2 * x[:,0]
        a1[:,1,1] = 2 * self.sigma2 ** 2 * x[:,1]
        a1[:,2,2] = 2 * self.sigma3 ** 2 * x[:,2]

        a2 = torch.zeros((n_points, self.state_dim, self.state_dim), dtype=x.dtype, device=device)
        a2[:,0,0] = 2 * self.sigma1 ** 2
        a2[:,1,1] = 2 * self.sigma2 ** 2
        a2[:,2,2] = 2 * self.sigma3 ** 2

        a3 = torch.zeros((n_points, self.state_dim), dtype=x.dtype, device=device)
        a3[:,0] = (self.alpha1 - self.alpha2 * x[:,2])
        a3[:,1] = (self.beta1 - self.beta2 * x[:,2])
        a3[:,2] = (self.alpha2 * x[:,0] + self.beta2 * x[:,1] - self.gamma)

        a4 = self.drift_batch_torch(x)

        H = torch.zeros((self.meas_dim, self.state_dim), dtype=x.dtype, device=device)
        H[0,0] = 1
        H[1,1] = 1
        H[2,2] = 1
        H = H.unsqueeze(0).repeat(n_points, 1, 1)

        return a1, a2, a3,  a4, H
    
    def simulate_starting_state(self, N_simulations):
        x1 = np.random.normal(self.R0, self.R0_std, (N_simulations, 1))
        x2 = np.random.normal(self.M0, self.M0_std, (N_simulations, 1))
        x3 = np.random.normal(self.F0, self.F0_std, (N_simulations, 1))
        return np.asarray([x1, x2, x3]).T

    def get_prior_moments(self):
        """ 
        Returns mean and covariance for the prior distribution
        Needed for all kalman filters
        """
        P0 = np.identity(self.state_dim)
        P0[0,0] = P0[0,0] * self.R0_std ** 2
        P0[1,1] = P0[1,1] * self.M0_std ** 2
        P0[2,2] = P0[2,2] * self.F0_std ** 2
        m0 = np.zeros(self.state_dim)
        m0[0] = self.R0
        m0[1] = self.M0
        m0[2] = self.F0
        return m0, P0

    def prior_likelihood(self, x):
        """
        Returns the likelihood of a given starting state (pdf of the prior)
        Input:
        x: (num_points, state_dim) or (state_dim)
        """
        P0 = np.identity(self.state_dim)
        P0[0,0] = P0[0,0] * self.R0_std ** 2
        P0[1,1] = P0[1,1] * self.M0_std ** 2
        P0[2,2] = P0[2,2] * self.F0_std ** 2
        m0 = np.zeros(self.state_dim)
        m0[0] = self.R0
        m0[1] = self.M0
        m0[2] = self.F0

        mvn = multivariate_normal(mean=m0, cov=P0)
        likelihood = mvn.pdf(x)
        return likelihood

    def prior_likelihood_torch(self, x):
        """
        Returns the likelihood of a given starting state (pdf of the prior)
        Input:
        x: (num_points, state_dim) or (state_dim)
        """
        P0 = torch.eye(self.state_dim, dtype=x.dtype, device=device)
        P0[0,0] = P0[0,0] * self.R0_std ** 2
        P0[1,1] = P0[1,1] * self.M0_std ** 2
        P0[2,2] = P0[2,2] * self.F0_std ** 2
        m0 = torch.zeros(self.state_dim, dtype=x.dtype, device=device)
        m0[0] = self.R0
        m0[1] = self.M0
        m0[2] = self.F0

        mvn = torch.distributions.multivariate_normal.MultivariateNormal(loc=m0, covariance_matrix = P0)

        # Encountered cuda error with too many samples
        max_batch_size = 10 ** 5
        if x.size(dim = 0) <= max_batch_size:
            likelihood = torch.exp(mvn.log_prob(x))
        else:
            n_points = x.size(dim=0)
            likelihood = torch.zeros((n_points, 1), device=device)
            n_iter = n_points // max_batch_size
            for i in range(n_iter):
                start_idx = max_batch_size * i
                end_idx = min(max_batch_size * (i + 1), n_points)
                likelihood[start_idx:end_idx, :] = torch.exp(mvn.log_prob(x[start_idx:end_idx, :])).view(-1, 1)
            likelihood = likelihood.flatten()

        return likelihood

    def prior_gradient_torch(self, x):
        """
        Returns the gradient of the prior in given points.
        Also returns the likelihood, which allows for more efficient code down the line
        Needed for energy-based deep splitting
        """

        P0 = torch.eye(self.state_dim, dtype=x.dtype, device=device)
        P0[0,0] = P0[0,0] * self.R0_std ** 2
        P0[1,1] = P0[1,1] * self.M0_std ** 2
        P0[2,2] = P0[2,2] * self.F0_std ** 2
        m0 = torch.zeros(self.state_dim, dtype=x.dtype, device=device)
        m0[0] = self.R0
        m0[1] = self.M0
        m0[2] = self.F0

        likelihood = self.prior_likelihood_torch(x).view(-1, 1)
        gradient = (-torch.matmul(torch.linalg.inv(P0), (x - m0).t()) * likelihood.t()).t()
        return gradient, likelihood
