...
# Defines an Ornstein-Uhlenbeck SDE in 5 dimensions with linear measurements
# dXt = -Q Xt dt + dWt
# h(x) = x 

class OrnsteinUhlenbeck5d(DiffusionSDE):
    def __init__(self):
        self.identifier = "OU5D"

        self.measurement_noise_std = 1
        self.state_dim = 5
        self.meas_dim = 5
        self.Q = 5 * np.eye(self.state_dim)
        self.Q_torch = torch.tensor(self.Q, dtype = torch.float32, device=device)

        self.invertible_meas = True

    def drift(self, x):
        mu = -np.matmul(self.Q, x)
        return mu
    
    def drift_batch(self, x):
        mu = -np.matmul(x, self.Q.T)
        return mu
    
    def drift_batch_torch(self, x):
        mu = -torch.matmul(x, self.Q_torch.t())
        return mu
    ...
    def get_kalman_filter_params(self, deltat):
        ...
        r_k ~ R_k 
        """
        A = np.identity(self.state_dim) - self.Q*deltat
