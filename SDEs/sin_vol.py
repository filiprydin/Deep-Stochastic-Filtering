...
class SinVol(DiffusionSDE):
    ...
    def drift(self, x):
        ...
        mu[0] = -self.alpha1 * np.sin(x[0])
        mu[1] = -self.alpha2 * x[1] ** 3
        return mu
    ...
    def drift_batch_torch(self, x):
        ...
        mu[:,0] = -self.alpha1 * torch.sin(x[:,0])
        mu[:,1] = -self.alpha2 * torch.pow(x[:,1], 3)
        return mu

    def diffusion(self, x):
        sigma = np.zeros((self.state_dim, self.state_dim))
        sigma[0,0] = self.sigma1 * np.sqrt(0.1 + x[1] ** 2)
        sigma[1,1] = self.sigma2
        return sigma
    ...
    def get_extended_kalman_filter_pred_params(self, x, q, deltat):
        ...
        Fx[0,0] += -self.alpha1 * np.cos(x[0]) * deltat
        Fx[1,1] += -3 * self.alpha2 * x[1] ** 2 * deltat
        Fq = self.diffusion(x)
    ...
    def get_ebds_filter_params(self, x):
        ...
        a3[:,0] = -self.alpha1 * torch.cos(x[:,0])
        a3[:,1] = -3 * self.alpha2 * torch.pow(x[:,1], 2)
    ...
    def simulate_starting_state(self, N_simulations):
        x1 = np.random.normal(0, self.p0_std, (N_simulations, 1))
        x2 = np.random.normal(0, 1, (N_simulations, 1))
