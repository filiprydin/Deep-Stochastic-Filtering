...
# Defines a bimodal SDE with linear measurements
# dXt = b(aXt - Xt^3) dt + dWt
# h(x) = x 

class BiModal(DiffusionSDE):
    def __init__(self):
        self.identifier = "BI"
    ...
    def drift(self, x):
        ...
        mu[0] = self.b * (self.a * x[0] - x[0] ** 3)
        return mu
    
    def drift_batch(self, x):
        mu = np.zeros_like(x)
        mu[:, 0] = self.b * (self.a * x[:,0] - np.power(x[:,0], 3))
        return mu
    
    def drift_batch_torch(self, x):
        mu = torch.zeros_like(x, device=device)
        mu[:, 0] = self.b * (self.a * x[:,0] - torch.pow(x[:,0], 3))
        return mu
    ...
    def get_extended_kalman_filter_pred_params(self, x, q, deltat):
        ...
        Fx[0] += self.b * (self.a - 3 * x[0] ** 2) * deltat
        Fq = self.diffusion(x)
