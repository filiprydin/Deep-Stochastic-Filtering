...
# Defines a geometric brownian motion SDE with linear measurements
# dXt = mu Xt dt + sigma Xt dWt
# h(x) = x 

class GeometricBrownianMotion(DiffusionSDE):
    def __init__(self):
        self.identifier = "GBM"
    ...
    def simulate_starting_state(self, N_simulations):
        X_0 = np.abs(np.random.normal(0, 1, (N_simulations, self.state_dim)))
        return X_0
    ...
    def get_prior_moments(self):
        ...
        m0[0] = np.sqrt(2/np.pi)
        P0[0] = 1 - np.sqrt(2/np.pi)
        return m0, P0
    ...
    def prior_likelihood(self, x):
        ...
        likelihood = 2 * mvn.pdf(x) * np.all(x > 0, axis=1)
        return likelihood
    ...
    def prior_likelihood_torch(self, x):
        ...
        likelihood = 2 * likelihood * torch.all(x > 0, dim = 1)
        return likelihood
